...
namespace FitAPIFunctions
{
    ...
    {
        private readonly ILogger<mergePerson> _logger;

        public mergePerson(ILogger<mergePerson> logger)
        {
            _logger = logger;
        }
	    [Function("mergePerson")]
        public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Function, "get", "post", Route = "merge/Person")]HttpRequestData req)
		{
			...
			using (SqlConnection conn = new SqlConnection(sqlConnectionString))
                {
                    ...
                    new
                    {
						...
						, new List<string>(new string[] { "Active", "CreatedAt", "DateOfBirth", "DateOfBirthID", "Deleted", "FirstName", "Height", "ID", "PersonID", "Surname", "UpdatedAt", "Version" })
						)
                    },
