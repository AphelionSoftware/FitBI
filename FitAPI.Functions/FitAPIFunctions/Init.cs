...
namespace FitAPIFunctions
{
    ...
    {
        [FunctionName("Init")]
        public static HttpResponseMessage Run([HttpTrigger(AuthorizationLevel.Function, "get", "post", Route = "setup/Init/{pSubscriptionID}")]HttpRequestMessage req, string pSubscriptionID, TraceWriter log)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    using (var multi = conn.QueryMultiple(sp_Init, new { Subject = pSubscriptionID },
